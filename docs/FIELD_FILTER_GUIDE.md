...
字段名所在行与全项目一致，为 **`FIELD_NAME_ROW`**（默认第 5 行），见 [EXCEL_TABLE_LAYOUT.md](EXCEL_TABLE_LAYOUT.md)。
...
对 `excluded_field_names` 中的字段名：在数据区抽样若干格，**仅当某格含中文、越南文或泰文时保留该列**；否则跳过（视为代码/ID 列）。

```python
# 逻辑摘要（见 core/excel_field_extractor.py 实现）
for col_num in sorted(text_columns):
    field_name = str(field_cell.value).strip().lower()
    if field_name in self.excluded_field_names:
        if not column_has_cjk_viet_or_thai_sample(sheet, col_num, ...):
            continue
...
2. **基于字段名**: 只检查 **`FIELD_NAME_ROW`**（默认第 5 行）上的字段名称
