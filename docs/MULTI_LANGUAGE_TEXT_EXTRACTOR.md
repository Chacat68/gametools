...
> 与当前统一 GUI 中的「多语言翻译提取」流程对齐的 **策划表行号、列标记、行边界** 约定，见 [EXCEL_TABLE_LAYOUT.md](EXCEL_TABLE_LAYOUT.md) 与 `core/constants.py`。下文若仍写「第 5/6/7 行」，均指默认下的 `FIELD_NAME_ROW` / `FIELD_TYPE_ROW` / `DATA_START_ROW`。
...
2. **各工作表文本列表**: 每个工作表的提取文本，包含以下列：
   - **id列**: 原Excel文件的A列内容
   - **位置列**: Excel物理位置引用（如F7）
   - **字段名列**: 原 Excel **`FIELD_NAME_ROW`**（默认第 5 行）的字段名
