...
工具使用 Python 标准库实现，检测流程如下：

1. **统一读取** - 按 `utf-8`、`utf-8-sig`、`gbk` 等编码尝试读取，每个文件只读一次
2. **标准解析优先** - 先执行 `json.loads()`，以解析器报错作为首要依据
3. **字符串感知启发式** - 仅在双引号字符串外部检测尾随逗号、单引号、注释，避免将 `http://` 等合法内容误判为错误
4. **结构化结果** - `detect_file()` / `detect_folder()` 返回结构化数据，文本报告由 `generate_report()` / `generate_folder_report()` 生成
