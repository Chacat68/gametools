...
- **工作台路径同步**：「工作台」页集中选择常用目录与文件路径，与各功能页输入框共用变量，避免重复浏览选择
...
以下为 **统一工作台**（`python gui/run_unified.py` 或打包 exe）侧栏页签说明。启动后默认打开 **「工作台」** 页：可在此一次性选择各语言表目录、统一输出目录、合并 JSON、JSON 检测目录、Excel 与批量改表相关路径等；这些输入框与各功能页中的对应路径 **绑定同一变量**，任一处修改会立即同步到其它页，无需重复选择。
...
- 可在「关于 → 界面与模块设置」中关闭「工作台」页签（关闭后路径仍会在各功能页保留与同步逻辑，仅隐藏集中入口）。

### 字段导出

1. 为中文、越南语、泰语、英语等勾选「导出」并分别选择 Excel 所在目录（与表内 `_zh` / `_vn` 等后缀一致）。
2. 选择输出目录；输出格式选 **JSON** 时会在输出目录生成合并文件（默认名见 `core.constants.FIELD_EXTRACTION_MERGED_JSON_NAME`）。
3. 点击「开始提取」；完成后若存在合并 JSON，可使用 **「用于多语言提取」** 一键填好多语言页并跳转。

### 多语言提取

1. 选择合并 JSON（可由字段导出生成，或自行准备同结构文件）；各语言目录与输出目录若已在 **工作台** 填过，此处会自动显示相同路径。
2. 点击开始提取，生成翻译总表等结果。

### 批量改表

1. 选择映射表、JSON 配置、目标 Excel 目录等选项。
2. 执行批量修改；详细行为见 [docs/BATCH_MODIFIER_GUIDE.md](docs/BATCH_MODIFIER_GUIDE.md)。

### 跨项目翻译

按映射加载、扫描项目目录并输出对齐结果（与上列流水线并行，无强制先后顺序）。

### JSON 错误检测工具页签

1. **选择路径**：点击「浏览文件夹」按钮选择包含 JSON 文件的文件夹
...
1. **选择文件**：点击「浏览」按钮选择要处理的 Excel 文件
